        ...
        @if($showModal)
            <div class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 transition-opacity duration-300">
                <div class="bg-white rounded-lg p-6 w-full max-w-md mx-4 sm:mx-auto transform transition-all">
                    <h2 class="text-xl font-semibold text-gray-800 mb-4">{{ $isEditing ? 'Edit Tag' : 'Add Tag' }}</h2>
            ...
            <table class="w-full border-collapse bg-white rounded-lg shadow-sm">
                ...
                @forelse($tags as $tag)
                    <tr class="border-b hover:bg-gray-50 transition">
                        <td class="p-3 text-gray-800">{{ $tag->name }}</td>
                        <td class="p-3">
                            <div class="flex space-x-2">
                                <button wire:click="edit({{ $tag->id }})" class="text-blue-600 hover:text-blue-800 flex items-center">
                                ...
                                <button wire:click="delete({{ $tag->id }})" wire:confirm="Are you sure?" class="text-red-600 hover:text-red-800 flex items-center">
                ...
                @empty
                    <tr>
                        <td colspan="2" class="p-3 text-center text-gray-500">No tags found.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
